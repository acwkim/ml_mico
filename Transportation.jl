...
#solve the transportation optimization problem, given the appropriate parameters.
function transportation(s,d,c_matrix,gurobi_env)
    n,m = size(c_matrix)


    final = Model(optimizer_with_attributes(() -> Gurobi.Optimizer(gurobi_env), "OutputFlag" => 0))
    @variable(final ,x[1:n, 1:m] >=0)
    @constraint(final, [i = 1:n], sum(x[i,k] for k = 1:m) <= s[i])
    @constraint(final, [k = 1:m], sum(x[i,k] for i = 1:n) >= d[k])
    @objective(final, Min, sum(c_matrix[i,k] * x[i,k] for i = 1:n, k= 1:m))
    ...
    tight_1 = tight_x #This is the optimal strategy

    return base_val, tight_1
end

#Solve the transportation optimziation problem, given a strategy st.
function transportation_st(s,d,c_matrix,st, gurobi_env)
    #apply strategy st to transporation optimization.
    #The penalty M is set to 1e4 in this code.

    n,m = size(c_matrix)
    idx_1 = findall(!iszero, st[1:n+m])
    idx_2 = findall(!iszero, st[n+m+1:end])
    Matx1 = Matrix(I, m, m)
    for i = 1:n-1
        mat = Matrix(I, m, m)
        Matx1 = hcat(Matx1,mat)
...
function sample_generating(n,m, s_num, gurobi_env, r)
    ...
    for i = 1:s_num
        ...
        opt_val, tight = transportation(s,d,c_matrix,gurobi_env)
    ...
    return feature, opt_x, optval, s,c_matrix
end

#N is the number of training set + test set
feature, opt_x, optval, s,c_matrix = sample_generating(n,m,N,gurobi_env, r)
...
for l = 1:num_train
        ...
        for k = 1:numu
            strategy = list[k]
            real_val = transportation_st(s,d,c_matrix,strategy, gurobi_env)
            Reward[l,k] = real_val
        end
        if l % 100 == 0
            println(l)
